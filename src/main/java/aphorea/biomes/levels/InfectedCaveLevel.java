...
public class InfectedCaveLevel extends InfectedSurfaceLevel {
    ...
    public void generateLevel() {
        ...
        GameEvents.triggerEvent(new GenerateCaveMiniBiomesEvent(this, cg), (e) -> {
            ...
            cg.generateRandomSingleRocks(ObjectRegistry.getObjectID("infectedrubyclustersmall"), 0.01F);
            cg.generateRandomSingleRocks(ObjectRegistry.getObjectID("rubyclustersmall"), 0.01F);
            cg.generateRandomSingleRocks(ObjectRegistry.getObjectID("infectedrubyclusterpure"), 0.002F);
            cg.generateRandomSingleRocks(ObjectRegistry.getObjectID("rubyclusterpure"), 0.002F);
        });
        ...
        GameObject crystalClusterSmall2 = ObjectRegistry.getObject("infectedrubyclustersmall");
            ...
            ca.streamAliveOrdered().forEachOrdered((tile) -> {
                if (this.getObjectID(tile.x, tile.y) == 0 && this.getObjectID(tile.x - 1, tile.y) == 0 && this.getObjectID(tile.x + 1, tile.y) == 0 && this.getObjectID(tile.x, tile.y - 1) == 0 && this.getObjectID(tile.x, tile.y + 1) == 0 && cg.random.getChance(0.08F)) {
                    ...
                    if (this.getRelativeAnd(tile.x, tile.y, PresetUtils.getRotatedPoints(0, 0, rotation, clearPoints), (tileX, tileY) -> {
                        ...
                        ObjectRegistry.getObject(ObjectRegistry.getObjectID((GameRandom.globalRandom.getChance(0.5) ? "infected" : "") + "rubycluster")).placeObject(this, tile.x, tile.y, rotation, false);
                    }
                }

                if (cg.random.getChance(0.3F) && crystalClusterSmall.canPlace(this, tile.x, tile.y, 0, false) == null) {
                    (GameRandom.globalRandom.getChance(0.5) ? crystalClusterSmall : crystalClusterSmall2).placeObject(this, tile.x, tile.y, 0, false);
                }
