package aphorea.items.weapons.range.saber.logic;

import aphorea.items.weapons.range.saber.AphSaberToolItem;
...
public class SaberDashAttackHandler extends MousePositionAttackHandler {
            ...
            this.hudDrawElement = this.attackerMob.getLevel().hudManager.addElement(new HudDrawElement() {
                public void addDrawables(List<SortedDrawable> list, GameCamera camera, PlayerMob perspective) {
                    if (SaberDashAttackHandler.this.attackerMob.getAttackHandler() != SaberDashAttackHandler.this) {
                        ...
                        if (distance > 0.0F) {
                            Point2D.Float dir = GameMath.normalize((float) SaberDashAttackHandler.this.lastX - SaberDashAttackHandler.this.attackerMob.x, (float) SaberDashAttackHandler.this.lastY - SaberDashAttackHandler.this.attackerMob.y);
                            final DrawOptions drawOptions = HUD.getArrowHitboxIndicator(SaberDashAttackHandler.this.attackerMob.x, SaberDashAttackHandler.this.attackerMob.y, dir.x, dir.y, (int) distance, 50, new Color(0, 0, 0, 0), new Color(220, 255, 255, 100), new Color(0, 0, 0, 100), camera);
