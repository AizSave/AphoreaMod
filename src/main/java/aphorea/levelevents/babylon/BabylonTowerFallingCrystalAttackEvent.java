...
public class BabylonTowerFallingCrystalAttackEvent extends MobAbilityLevelEvent implements Attacker {
    ...
    public void init() {
        super.init();
        if (this.isClient()) {
            this.level.entityManager.addParticle(new BabylonTowerFallingCrystalParticle(this.level, (float) this.x, (float) this.y, this.spawnTime, 1000L), Particle.GType.CRITICAL);
        }
    ...
    public void serverTick() {
        if (!this.isOver()) {
            ...
            if (eventTime > 1200L) {
                ...
                GameUtils.streamTargets(this.owner, GameUtils.rangeTileBounds(this.x, this.y, 5))
                        .filter((m) -> m.canBeHit(this.owner) && hitBox.intersects(m.getHitBox()))
                        .forEach((m) -> {
                            m.addBuff(new ActiveBuff(BuffRegistry.Debuffs.BROKEN_ARMOR, m, 5000, this), true);
